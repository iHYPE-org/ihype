...
const DOC = 'CLAUDE.md';
/* Only `src/` — a repo-relative path the reader is meant to open. `design/`
   and `templates/` are vendored bundles that legitimately come and go with a
   re-vendor, and `prisma/migrations-pending/` empties by design. */
const PATH_RE = /(src\/[a-zA-Z0-9_./[\]-]+\.(?:tsx?|mjs|mts|css|json|js))/g;
...
for (const row of rows) {
  const paths = [...new Set([...row.matchAll(PATH_RE)].map((m) => m[1]))];
  if (paths.length === 0) continue;
  /* MASK THE PATHS BEFORE ASKING WHETHER THE ROW SAYS "GONE". Testing the
     raw row lets a FILENAME satisfy the vocabulary — measured: a probe path
  ...
  const prose = row.replace(PATH_RE, ' ');
  const exempt = MARKER.test(prose) || SAYS_GONE.test(prose);
  if (exempt) {
    exemptRows += 1;
    if (LIST_EXEMPT) for (const p of paths) if (!existsSync(p)) exemptedDead.push({ p, row: row.split('|')[1].trim().slice(0, 60) });
    continue;
  }
  for (const p of paths) {
    considered += 1;
    if (!existsSync(p)) findings.push({ p, row: row.split('|')[1].trim().slice(0, 60) });
  }
}
...
if (considered < 100) {
  console.error(`audit:doc-paths — only ${considered} path(s) considered across ${rows.length} table row(s).`);
  console.error('That is implausibly low: the scan found nothing to check rather than nothing wrong.');
  process.exit(2);
}

console.log(`audit:doc-paths — ${considered} live-claimed src/ path(s) across ${rows.length} rows of ${DOC}.`);
console.log(`${exemptRows} row(s) name a path and say it is gone; those are not checked.`);
if (LIST_EXEMPT) {
  console.log(`\n${exemptedDead.length} dead path(s) inside those exempt rows — run this when the vocabulary feels too broad:\n`);
  for (const f of exemptedDead) console.log(`  ${f.p}\n      in the row headed: ${f.row}…`);
}

if (findings.length > max) {
  console.log(`\n${findings.length} path(s) named by a row that does NOT say the file is gone:\n`);
  ...
  console.log('gone, or add `doc-path-exempt: <reason>` to the row. Never delete the row silently —');
