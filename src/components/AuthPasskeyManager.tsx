...
export function PasskeyManager() {
  ...
  return (
    <div>
      ...
      {status ? <p style={{ marginTop: 8, fontSize: '0.9375rem', color: 'var(--accent-text)' }}>{status}</p> : null}
      {error ? <p style={{ marginTop: 8, fontSize: '0.9375rem', color: '#ef4444' }}>{error}</p> : null}

      {!loadingList && passkeys && passkeys.length > 0 ? (
        <div style={{ marginTop: 20 }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.9375rem', textTransform: 'uppercase', letterSpacing: '.08em', color: 'var(--ink-a65)', marginBottom: 10 }}>
          ...
          <div style={{ border: '1px solid var(--line)', borderRadius: 12, overflow: 'hidden' }}>
            {passkeys.map((pk, i) => (
              <div key={pk.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderBottom: i === passkeys.length - 1 ? 'none' : '1px solid var(--line)' }}>
                <span style={{ flex: 1, fontSize: '0.9375rem', color: 'var(--ink)' }}>
                ...
                <button
                  ...
                  style={{ ...ghostButton, padding: '6px 14px', fontSize: '0.9375rem', opacity: removingId === pk.id ? 0.7 : 1 }}
                  type="button"
                >
