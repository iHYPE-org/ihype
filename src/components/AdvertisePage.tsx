        ...
        {rows.map((item) => (
          <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '10px 11px', borderRadius: 9, animation: 'adv-rowIn .5s cubic-bezier(.2,.7,.2,1)' }}>
            ...
            <span style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontFamily: 'var(--f-b)', fontSize: 12.5, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.name}</div>
...
function CoverageBuilder() {
  ...
  const INPUT_S: React.CSSProperties = { fontFamily: 'var(--f-b)', fontSize: 'inherit', color: 'inherit', background: 'none', border: 'none', cursor: 'pointer', padding: 0 };
  const TEXT_INPUT_S: React.CSSProperties = { width: '100%', fontFamily: 'var(--f-b)', fontSize: 13, color: 'var(--ink)', background: 'var(--bg-3)', border: '1px solid var(--line-2)', borderRadius: 9, padding: '11px 13px', outline: 'none' };
    ...
    <div className="adv-builder" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, alignItems: 'stretch' }}>
      ...
      <div style={{ background: 'var(--bg-2)', border: '1px solid var(--hair-70)', borderRadius: 16, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '15px 20px', borderBottom: '1px solid var(--hair-70)', display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontFamily: 'var(--f-d)', fontWeight: 700, fontSize: 14, letterSpacing: '-.01em' }}>{t('advertisePage.campaign', 'Campaign')}</span>
        ...
        <div style={{ padding: '22px 20px', flex: 1 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
              {AD_SCOPES.map(s => (
                <button key={s} onClick={() => setScope(s)} style={{
                  ...
                  <span>
                    <div style={{ fontFamily: 'var(--f-d)', fontWeight: 700, fontSize: 15, letterSpacing: '-.01em' }}>{AD_SCOPE_LABELS[s]}</div>
                  ...
                  <span style={{ marginLeft: 'auto', textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ fontFamily: 'var(--f-d)', fontWeight: 700, fontSize: 16, letterSpacing: '-.01em', color: s === scope ? 'var(--accent)' : 'inherit' }}>{money(quoteAdCampaign(s, 1, 1).ratePerSpotCents / 100)}</div>
          ...
          <div style={{ marginBottom: 26 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 0, border: '1px solid var(--line-2)', borderRadius: 10, overflow: 'hidden', width: 'fit-content' }}>
              ...
              <div style={{ minWidth: 64, textAlign: 'center', fontFamily: 'var(--f-d)', fontWeight: 800, fontSize: 22, letterSpacing: '-.02em', borderLeft: '1px solid var(--hair-70)', borderRight: '1px solid var(--hair-70)', height: 46, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{spots}</div>
      ...
      <div style={{ background: 'var(--bg-2)', border: '1px solid var(--hair-70)', borderRadius: 16, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '15px 20px', borderBottom: '1px solid var(--hair-70)', display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontFamily: 'var(--f-d)', fontWeight: 700, fontSize: 14 }}>{AD_SCOPE_LABELS[scope]} {t('advertisePage.reach', 'reach')}</span>
        </div>
        <div style={{ padding: '22px 20px', flex: 1 }}>
          {/* Dot grid */}
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
            {[{ v: fmt(quote.dailyImpressions), l: t('advertisePage.dailyImpressions', 'Daily impressions') }, { v: fmt(quote.totalImpressions), l: t('advertisePage.totalOverRun', 'Total over run') }, { v: '$' + cpm.toFixed(2), l: t('advertisePage.effectiveCpm', 'Effective CPM') }].map(s => (
              <div key={s.l} style={{ flex: 1, padding: '13px 14px', border: '1px solid var(--hair-70)', borderRadius: 11, background: 'var(--bg-3)' }}>
                <div style={{ fontFamily: 'var(--f-d)', fontWeight: 800, fontSize: 21, letterSpacing: '-.02em' }}>{s.v}</div>
          ...
          <div style={{ marginTop: 20, borderTop: '1px dashed var(--line-2)', paddingTop: 18 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginTop: 12, paddingTop: 14, borderTop: '1px solid var(--hair-70)' }}>
              ...
              <span style={{ fontFamily: 'var(--f-d)', fontWeight: 800, fontSize: 34, letterSpacing: '-.03em', color: '#ff5029' }}>
            ...
            {submit.phase === 'done' && (
              <div style={{
                ...
                <div style={{ fontFamily: 'var(--f-d)', fontWeight: 800, fontSize: 13, color: submit.status === 'AWAITING_PAYMENT' ? '#22e5d4' : submit.status === 'REJECTED' ? '#ff5a5a' : '#ffb84a' }}>
                  {submit.message}
                </div>
    ...
    <div className="adv-guard" style={{ display: 'grid', gridTemplateColumns: '.86fr 1.14fr', gap: 18, alignItems: 'start' }} ref={wrapperRef}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {[
          ...
          <div key={c.n} style={{ display: 'flex', gap: 14, padding: '18px', border: '1px solid var(--hair-70)', borderRadius: 14, background: 'var(--bg-2)' }}>
            ...
            <div>
              <div style={{ fontFamily: 'var(--f-d)', fontWeight: 700, fontSize: 15, letterSpacing: '-.01em' }}>{c.title}</div>
      ...
      <div style={{ background: 'var(--bg-2)', border: '1px solid var(--hair-70)', borderRadius: 16, overflow: 'hidden' }}>
        <div style={{ padding: '15px 18px', borderBottom: '1px solid var(--hair-70)', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontFamily: 'var(--f-d)', fontWeight: 700, fontSize: 14 }}>{t('advertisePage.submissionReview', 'Submission review')}</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '188px 1fr', minHeight: 330 }}>
          ...
          <div style={{ borderRight: '1px solid var(--hair-70)', padding: 10, display: 'flex', flexDirection: 'column', gap: 6 }}>
            {SUBS.map((s, i) => (
              <button key={i} onClick={() => runScan(i)} style={{
                ...
                <div style={{ fontFamily: 'var(--f-b)', fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{s.n}</div>
          ...
          <div style={{ padding: '18px 20px', position: 'relative', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ border: '1px solid var(--hair-70)', borderRadius: 11, padding: '14px 16px', background: 'var(--bg-3)', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ fontFamily: 'var(--f-d)', fontWeight: 700, fontSize: 17, letterSpacing: '-.01em', marginTop: 8, lineHeight: 1.15 }}>{sub.copy}</div>
              ...
              {sub.gates.map((g, i) => {
                ...
                return (
                  <div key={i} style={{
                    ...
                    <span style={{ flex: 1 }}>
                      <div style={{ fontFamily: 'var(--f-b)', fontSize: 12.5, fontWeight: 600 }}>{g[0]}</div>
  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto', background: 'var(--bg-2)', border: '1px solid var(--hair-100)', borderRadius: 14, overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '13px 18px', borderBottom: '1px solid var(--hair-70)' }}>
        <span style={{ fontFamily: 'var(--f-d)', fontWeight: 700, fontSize: 14, flex: 1 }}>{t('advertisePage.hypeScreenAutomated', 'HYPE Screen · automated ad review')}</span>
        ...
        <div style={{ padding: '0 18px 18px', display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          {phase === 'done' && (
            <div style={{ marginTop: 6, padding: '13px 16px', borderRadius: 10, background: 'rgba(var(--role-venue-rgb),.1)', border: '1px solid rgba(var(--role-venue-rgb),.3)', textAlign: 'center' }}>
              <div style={{ fontFamily: 'var(--f-d)', fontWeight: 800, fontSize: 15, color: 'var(--role-venue)', marginBottom: 3 }}>{t('advertisePage.clearedCheckout', '✓ CLEARED — checkout unlocked')}</div>
      ...
      <header id="top" style={{ position: 'relative', padding: '40px 0 84px', overflow: 'hidden' }}>
        ...
        <div className="adv-hero-grid adv-shell" style={{ width: '100%', maxWidth: 1180, margin: '0 auto', padding: '0 40px', display: 'grid', gridTemplateColumns: '1.18fr .92fr', gap: 54, alignItems: 'center', position: 'relative' }}>
          <div>
            ...
            <h1 className="adv-hero-title" style={{ fontFamily: 'var(--f-d)', fontWeight: 800, fontSize: 68, lineHeight: .96, letterSpacing: '-.035em', margin: '20px 0 0' }}>
            ...
            <div style={{ display: 'flex', gap: 26, marginTop: 34, paddingTop: 24, borderTop: '1px solid var(--hair-70)' }}>
              {[
                ...
                <div key={m.l}>
                  <div style={{ fontFamily: 'var(--f-d)', fontWeight: 800, fontSize: 24, letterSpacing: '-.02em' }}>{m.v}</div>
      ...
      <section id="build" style={{ position: 'relative', padding: '88px 0', borderTop: '1px solid var(--hair-70)' }}>
        <div className="adv-shell" style={{ width: '100%', maxWidth: 1180, margin: '0 auto', padding: '0 40px' }}>
          <div style={{ maxWidth: 680, marginBottom: 40 }}>
            ...
            <h2 style={{ fontFamily: 'var(--f-d)', fontWeight: 800, fontSize: 38, letterSpacing: '-.03em', lineHeight: 1.02, margin: '14px 0 0' }}>
        ...
        <div className="adv-shell" style={{ width: '100%', maxWidth: 1180, margin: '0 auto', padding: '0 40px' }}>
          ...
          <div className="adv-paths" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
            ...
            <div style={{ border: '1px solid var(--hair-70)', borderRadius: 16, background: 'var(--bg-2)', padding: '28px 28px 26px', display: 'flex', flexDirection: 'column' }}>
              ...
              <h3 style={{ fontFamily: 'var(--f-d)', fontWeight: 800, fontSize: 26, letterSpacing: '-.02em', marginTop: 16, lineHeight: 1.05 }}>{t('advertisePage.artistsVenues', 'Artists, venues')}<br />{t('advertisePage.promotersLine', '& promoters')}</h3>
            ...
            <div style={{ border: '1px solid var(--hair-70)', borderRadius: 16, background: 'var(--bg-2)', padding: '28px 28px 26px', display: 'flex', flexDirection: 'column' }}>
              ...
              <h3 style={{ fontFamily: 'var(--f-d)', fontWeight: 800, fontSize: 26, letterSpacing: '-.02em', marginTop: 16, lineHeight: 1.05 }}>{t('advertisePage.musicBusinesses', 'Music businesses')}<br />{t('advertisePage.suppliersLine', '& suppliers')}</h3>
        ...
        <div className="adv-shell" style={{ width: '100%', maxWidth: 1180, margin: '0 auto', padding: '0 40px' }}>
          ...
          <div className="adv-trans" style={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: 40, alignItems: 'center', marginTop: 8 }}>
            <p style={{ fontFamily: 'var(--font-serif-accent)', fontStyle: 'italic', fontSize: 26, lineHeight: 1.35, color: 'var(--ink-2)', maxWidth: '22ch' }}>
              {t('advertisePage.transBodyPrefix', 'iHYPE is run by one director and a lot of automation — so almost every dollar of ad spend')} <b style={{ fontFamily: 'var(--font-body)', fontStyle: 'normal', fontWeight: 600, color: 'var(--accent)' }}>{t('advertisePage.transBodyBold', 'goes back into the music')}</b>{t('advertisePage.transBodySuffix', ', not overhead.')}
            </p>
            <div style={{ borderTop: '1px solid var(--hair-70)' }}>
              {[{ lb: t('advertisePage.toArtistsPayouts', 'To artists & payouts'), w: '72%', color: 'var(--accent)', pct: '72%' }, { lb: t('advertisePage.platformHosting', 'Platform & hosting'), w: '18%', color: 'var(--ink)', pct: '18%' }, { lb: t('advertisePage.moderationAi', 'Moderation & AI'), w: '10%', color: 'var(--ink-2)', pct: '10%' }].map(r => (
                <div key={r.lb} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '16px 0', borderBottom: '1px solid var(--hair-70)' }}>
                  ...
                  <span style={{ fontFamily: 'var(--f-d)', fontWeight: 800, fontSize: 19, letterSpacing: '-.02em', width: 54, textAlign: 'right', color: r.color }}>{r.pct}</span>
                </div>
              ))}
            </div>
          </div>
