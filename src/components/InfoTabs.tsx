  ...
  return (
    <div className="legal-wrap">
      ...
      <style>{`
        ...
        /* …but NOT the two buttons, which are `<a>` elements inside this same
           panel and were being repainted by the prose-link rule above — it
           scores (0,1,1) against `.lp-btn-primary`'s (0,1,0) and wins.
           `--accent-text` is the accent used AS COPY and `--ink-on-accent` is
           ink ON a brand fill: inverses, never interchangeable, and painting
           the first onto the accent measures 2.37:1 against the 5.58:1 the
           right token gives. Caught by axe on /info, not by reading — the
           button's own rule is correct, and nothing in it hints that a link
           rule three hundred lines away outranks it. The ghost button is the
           same cause without the contrast failure: it was rendering accent
           where the design calls for muted. */
