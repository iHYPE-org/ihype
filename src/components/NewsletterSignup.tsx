...
export function NewsletterSignup({ fixedProfile }: { fixedProfile?: ProfileHit } = {}) {
  ...
  if (sent) {
    return (
      <div style={{ fontSize: '0.9375rem', color: 'var(--role-venue)', padding: '12px 16px', background: 'rgba(var(--role-venue-rgb),.08)', borderRadius: 8 }}>
      ...
      <div className="field" ref={containerRef} style={{ position: 'relative' }}>
        ...
        {profile ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'rgba(var(--accent-rgb),.07)', border: '1px solid rgba(var(--accent-rgb),.2)', borderRadius: 10 }}>
            <div style={{ flex: 1, fontSize: '0.9375rem', fontWeight: 600, color: 'var(--ink)' }}>{profile.name}</div>
