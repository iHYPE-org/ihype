  ...
  return (
    <>
      ...
      <div style={{ flex: 1, minWidth: 0, overflow: 'hidden' }}>
        <div style={{ fontFamily: 'var(--font-display)', fontSize: '0.9375rem', fontWeight: 800, letterSpacing: '-.01em', color: 'var(--ink)', overflowWrap: 'break-word', wordBreak: 'break-word' }}>
          {title}
        </div>
        <div style={{ fontSize: '0.9375rem', color: 'var(--ink-a65)', marginTop: 2, overflowWrap: 'break-word', wordBreak: 'break-word' }}>{sub}</div>
      </div>
...
function LoadNote({ text }: { text: string }) {
  return <p style={{ fontSize: '0.9375rem', color: 'var(--ink-a65)', margin: '8px 0 0' }}>{text}</p>;
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      {data.aiPlan && (
        <div style={{ borderLeft: '2px solid var(--accent)', paddingLeft: 12 }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.6875rem', letterSpacing: '.14em', textTransform: 'uppercase', color: 'var(--accent-text)', marginBottom: 6 }}>
            {t('pageRoleModules.aiRouteLabel', 'AI ROUTE')}
          </div>
          <p style={{ fontSize: '0.9375rem', color: 'var(--ink-a75)', margin: '0 0 10px', lineHeight: 1.5 }}>{data.aiPlan.summary}</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {data.aiPlan.route.map((r) => (
              <div key={`${r.order}-${r.city}`} style={{ display: 'flex', gap: 10 }}>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.9375rem', color: 'var(--accent-text)', flexShrink: 0, width: 18 }}>
                  {String(r.order).padStart(2, '0')}
                </span>
                <div style={{ minWidth: 0 }}>
                  <span style={{ fontSize: '0.9375rem', fontWeight: 700, color: 'var(--ink)' }}>
                    {r.city}{r.targetVenue ? <span style={{ fontWeight: 400, color: 'var(--ink-a65)' }}> · {r.targetVenue}</span> : null}
                  </span>
                  {r.why && <div style={{ fontSize: '0.9375rem', color: 'var(--ink-a65)', marginTop: 2, lineHeight: 1.45 }}>{r.why}</div>}
                </div>
              </div>
            ))}
          </div>
      ...
      <div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.6875rem', letterSpacing: '.14em', textTransform: 'uppercase', color: 'var(--ink-a65)', marginBottom: 8 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {data.stops.slice(0, 6).map((s) => (
            <div key={s.city} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '8px 0', borderBottom: '1px solid var(--line)' }}>
              <div style={{ minWidth: 0 }}>
                <span style={{ fontSize: '0.9375rem', fontWeight: 600, color: 'var(--ink)' }}>{s.city}</span>
                {s.venues[0] && <span style={{ fontSize: '0.9375rem', color: 'var(--ink-a65)' }}> · {s.venues.map((v) => v.name).slice(0, 2).join(', ')}</span>}
              </div>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.9375rem', color: 'var(--ink-a65)', flexShrink: 0 }}>
      ...
      {recs.map((r, i) => (
        <div key={i} style={{ border: '1px solid var(--hair-70)', borderRadius: 12, padding: '12px 14px', background: 'var(--hair-20)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.9375rem', letterSpacing: '.1em', textTransform: 'uppercase', padding: '2px 6px', borderRadius: 4, color: 'var(--role-fan)', background: 'rgba(var(--role-fan-rgb),.14)' }}>
              {r.channel}
            </span>
          </div>
          <div style={{ fontSize: '0.9375rem', fontWeight: 700, color: 'var(--ink)', marginBottom: 3 }}>{r.headline}</div>
          <div style={{ fontSize: '0.9375rem', color: 'var(--ink-a65)', lineHeight: 1.5 }}>{r.body}</div>
          <div style={{ fontSize: '0.9375rem', fontWeight: 600, color: 'var(--role-fan)', marginTop: 6 }}>{r.cta}</div>
        </div>
      ))}
      <Link href="/advertise" style={{ fontSize: '0.9375rem', color: 'var(--ink-a65)', textDecoration: 'underline', textUnderlineOffset: 3 }}>
