...
export function SupportPageClient() {
  ...
  function reportProblem() {
    ...
    setInitialSubject(t('supportPageClient.privacyConcernSubject', 'Privacy concern'));
    setFormKey((k) => k + 1);
    ...
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px,1fr))', gap: 14, marginBottom: 40 }}>
        <Link href="/tickets" style={quickCardStyle}>
          ...
          <div style={{ fontSize: 14, fontFamily: 'var(--font-display)', fontWeight: 800, marginBottom: 4 }}>{t('supportPageClient.myTickets', 'My Tickets')}</div>
          <div style={{ fontSize: 12, color: 'var(--ink-a55)' }}>{t('supportPageClient.myTicketsDescription', 'Transfer, share, or get a QR code')}</div>
        </Link>
        ...
        <Link href="/verify" style={quickCardStyle}>
          ...
          <div style={{ fontSize: 14, fontFamily: 'var(--font-display)', fontWeight: 800, marginBottom: 4 }}>{t('supportPageClient.verification', 'Verification')}</div>
          <div style={{ fontSize: 12, color: 'var(--ink-a55)' }}>{t('supportPageClient.verificationDescription', 'Artist/venue/DJ verification status')}</div>
        </Link>
        <Link href="/me/promote" style={quickCardStyle}>
          <div style={{ fontSize: 24, marginBottom: 8 }} aria-hidden="true">💸</div>
          <div style={{ fontSize: 14, fontFamily: 'var(--font-display)', fontWeight: 800, marginBottom: 4 }}>{t('supportPageClient.payouts', 'Payouts')}</div>
          <div style={{ fontSize: 12, color: 'var(--ink-a55)' }}>{t('supportPageClient.payoutsDescription', 'Settlement, earnings, referrals')}</div>
        </Link>
        <BugReportPanel />
      </div>
