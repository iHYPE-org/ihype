...
export function CollabBoardPostForm({ roles }: { roles: RoleOption[] }) {
  const { t } = useI18n();
  const TYPE_OPTIONS = [
    { value: 'looking-for', label: t('collabBoardPostForm.lookingFor', 'Looking for') },
    { value: 'available', label: t('collabBoardPostForm.available', 'Available') },
  ];
  ...
  async function submit(e: FormEvent) {
    ...
    setError('');
    if (!body.trim()) {
      setError(t('collabBoardPostForm.descriptionRequired', 'Description is required.'));
      return;
    }
    setSubmitting(true);
    try {
      ...
      if (!res.ok) {
        setError(data.error ?? t('collabBoardPostForm.postError', 'Could not post that listing.'));
        return;
      }
      setBody('');
      setContact('');
      ...
      setError(t('collabBoardPostForm.postError', 'Could not post that listing.'));
    } finally {
      setSubmitting(false);
    }
  }
  ...
  if (!open) {
    return (
      <button type="button" className="ihype-btn-primary" onClick={() => setOpen(true)} style={{ display: 'inline-block' }}>
        + {t('collabBoardPostForm.postAListing', 'Post a listing')}
      </button>
    );
  }
  ...
  return (
    <form onSubmit={submit} className="ihype-card" style={{ padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <textarea
        ...
        placeholder={t('collabBoardPostForm.bodyPlaceholder', "Tell people what you're looking for or what you have to offer…")}
        required
        rows={4}
        maxLength={500}
        value={body}
      ...
      <input
        className="ihype-input"
        placeholder={t('collabBoardPostForm.contactPlaceholder', 'Contact (email, phone, or handle) — optional')}
        maxLength={100}
        value={contact}
        onChange={(e) => setContact(e.target.value)}
      />
      {error && <p style={{ fontSize: '0.8125rem', color: 'var(--accent-text)', margin: 0 }}>{error}</p>}
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <button type="button" className="ihype-btn-ghost" onClick={() => setOpen(false)}>
          {t('collabBoardPostForm.cancel', 'Cancel')}
        </button>
      </div>
    </form>
  );
