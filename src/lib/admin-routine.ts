...
export const AUTOMATED_JOBS: readonly AutomatedJob[] = [
  ...
  { path: '/api/cron?job=stripe-reconcile', schedule: '0 5 * * *', label: 'Stripe reconciliation', what: 'Compares a week of PaymentIntents (platform and venue accounts) with ticket orders and campaigns, and released payables with the Stripe transfers that paid them, and emails the administrators when the two ledgers disagree about money — reports, never changes a row', aliveKey: 'stripe-reconcile' },
