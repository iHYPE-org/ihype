...
export async function sendArtistEarningsSummaryEmail(user: {
  ...
  return sendMarketingEmail(user.id, {
    ...
    text: `Hey ${name}, here's your iHYPE artist summary:\n\n• $${earnings} in pending earnings\n• ${user.trackCount} tracks live\n• ${user.hypeCount7d} hypes this week\n\nLog in to see your full dashboard: https://ihype.org/app/me/payouts`,
    html: `<p>Hey ${escapeHtml(name)}, here's your iHYPE artist summary:</p><ul><li><strong>$${earnings}</strong> in pending earnings</li><li><strong>${user.trackCount}</strong> tracks live</li><li><strong>${user.hypeCount7d}</strong> hypes this week</li></ul><p><a href="https://ihype.org/app/me/payouts">View your dashboard →</a></p>`,
  });
}
