    ...
    (
      <div style={OG_FRAME}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {genre ? <div style={{ ...OG_KICKER, color: OG.accent }}>{genre}</div> : null}
          <div style={{ fontSize: 80, fontWeight: 800, color: OG.ink, lineHeight: 0.98, letterSpacing: '-0.04em' }}>{name}</div>
          <div style={{ display: 'flex', gap: 24, alignItems: 'center' }}>
            {location ? <span style={{ fontSize: 24, color: OG.ink3 }}>{location}</span> : null}
            {hype ? <span style={{ fontSize: 24, color: OG.accent, fontWeight: 700 }}>{hype}</span> : null}
          </div>
        </div>
