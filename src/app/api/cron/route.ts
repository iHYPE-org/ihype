      ...
      if (snapshot.status !== 'ok' && isEmailDeliveryConfigured()) {
        try {
          ...
          await sendGenericEmail({
            ...
            html: `<p>iHYPE health check returned non-ok status.</p><pre style="font-family:monospace;font-size:12px;background:#0b1220;color:#eef1f6;padding:12px;border-radius:6px;white-space:pre-wrap;">${summary.replace(/&/g, '&amp;').replace(/</g, '&lt;')}</pre>`
          });
        } catch (err) {
