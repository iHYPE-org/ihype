        ...
        <>
          <form method="get" style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16, alignItems: 'flex-end' }}>
            ...
            <label style={{ display: 'grid', gap: 2 }}>
              <span className="meta" style={{ fontSize: '0.9375rem' }}>{t('adminReviewPage.from', 'From')}</span>
              <input className="input" type="date" name="from" defaultValue={resolved.from ?? ''} />
            </label>
            <label style={{ display: 'grid', gap: 2 }}>
              <span className="meta" style={{ fontSize: '0.9375rem' }}>{t('adminReviewPage.to', 'To')}</span>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <p style={{ fontFamily: 'var(--f-m)', fontSize: '0.9375rem', color: 'var(--ink-2)', margin: 0 }}>{reportsTotal} {rStatus.toLowerCase()} {reportsTotal !== 1 ? t('adminReviewPage.reportsPlural', 'reports') : t('adminReviewPage.reportSingular', 'report')}</p>
          ...
          {reports.length === 0 ? (
            <p style={{ fontFamily: 'var(--f-m)', fontSize: '0.9375rem', color: 'var(--ink-3)' }}>{t('adminReviewPage.noReports', 'No reports found.')}</p>
          ) : (
                  ...
                  {reports.map(r => {
                    ...
                    return (
                      <tr key={r.id} style={{ borderBottom: '1px solid var(--line)' }}>
                        ...
                        <td style={{ ...tdStyle, fontSize: '0.9375rem' }}>
                          {entityHref ? <Link href={entityHref} style={{ color: 'var(--accent-text)', textDecoration: 'none' }} target="_blank">{entityLabel}</Link> : <span style={{ fontFamily: 'monospace', fontSize: '0.9375rem', color: 'var(--ink-3)' }}>{entityLabel}</span>}
                        </td>
                        ...
                        <td style={tdStyle}>
                          <span style={{ padding: '2px 8px', borderRadius: 4, fontSize: '0.9375rem', letterSpacing: '.08em', background: r.status === 'OPEN' ? 'rgba(var(--accent-rgb),.15)' : 'rgba(var(--role-venue-rgb),.1)', color: r.status === 'OPEN' ? 'var(--accent-text)' : 'var(--role-venue)' }}>{r.status}</span>
                        </td>
                        <td style={tdStyle}>
          ...
          {reportPages > 1 && (
            <div style={{ display: 'flex', gap: 8, marginTop: 16, alignItems: 'center' }}>
              ...
              <span style={{ fontFamily: 'var(--f-m)', fontSize: '0.9375rem', color: 'var(--ink-3)' }}>{t('adminReviewPage.pageOf', 'Page')} {rPage} {t('adminReviewPage.of', 'of')} {reportPages}</span>
              ...
              {[...profilesByName.entries()].map(([name, profiles]) => (
                <div key={name} className="panel" style={{ padding: '1rem' }}>
                  ...
                  <ul style={{ listStyle: 'none', padding: 0, display: 'grid', gap: 4 }}>
                    {profiles.map((p) => (
                      <li key={p.id} style={{ display: 'flex', gap: 12, alignItems: 'center', fontSize: '0.9375rem' }}>
                        <Link href={`/artists/${p.slug}`} target="_blank">{p.name}</Link>
                        <span className="badge" style={{ fontSize: '0.9375rem' }}>{p.type}</span>
