  ...
  return (
    <div className="container section admin-console">
      ...
      <section className="section">
              ...
              <div className="admin-list-row" key={ad.id} style={{ flexWrap: 'wrap', gap: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  <form method="POST" action={`/api/admin/ads/${ad.id}`} style={{ display: 'inline' }}>
                    ...
                    <button type="submit" className="button small" style={{ fontSize: 12 }}>{t('adminPage.approve', 'Approve')}</button>
                  </form>
                  <form method="POST" action={`/api/admin/ads/${ad.id}`} style={{ display: 'inline' }}>
                    <input type="hidden" name="status" value="REJECTED" />
                    <button type="submit" className="button small secondary" style={{ fontSize: 12 }}>{t('adminPage.reject', 'Reject')}</button>
                  </form>
                </div>
              </div>
            ))}
        ...
        <p className="meta" style={{ marginTop: 8 }}>
          {t('adminPage.manageViaAds', 'Manage via')} <code>PATCH /api/admin/ads/[adId]</code> {t('adminPage.withParams2', 'with')} <code>{`{status: "APPROVED"|"REJECTED"}`}</code>.
        </p>
      </section>
    </div>
  );
