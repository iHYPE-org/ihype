  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', color: '#f0f0f0', fontFamily: 'system-ui, sans-serif', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 24px' }}>
      <div style={{ maxWidth: 560, width: '100%' }}>
        <div style={{ fontFamily: 'monospace', fontSize: 11, letterSpacing: '.2em', color: 'var(--accent)', marginBottom: 16 }}>{t('auxQueueSlugPage.eyebrow', '● IHYPE · PASSED THE AUX')}</div>
        <h1 style={{ fontSize: 36, fontWeight: 800, letterSpacing: '-.02em', margin: '0 0 8px' }}>{queue.name}</h1>
        <p style={{ color: '#888', fontSize: 14, margin: '0 0 32px' }}>{queue.items.length} {t('auxQueueSlugPage.tracksSharedVia', 'tracks · shared via iHYPE')}</p>
        ...
        <div style={{ marginTop: 40, textAlign: 'center' }}>
          <a href="https://ihype.org" style={{ color: 'var(--accent)', fontSize: 13, textDecoration: 'none' }}>{t('auxQueueSlugPage.discoverMoreLink', 'Discover more on iHYPE →')}</a>
        </div>
      </div>
    </div>
  );
