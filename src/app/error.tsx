...
export default function Error({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div style={{ minHeight: '40vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem', textAlign: 'center' }}>
      ...
      {isDev && error.message && (
        <pre style={{ fontSize: '0.7813rem', textAlign: 'left', background: 'var(--bg-3)', padding: 12, borderRadius: 6, maxWidth: '100%', overflow: 'auto', whiteSpace: 'pre-wrap', wordBreak: 'break-word', marginBottom: 16 }}>
          {error.message}
          {error.stack ? `\n\n${error.stack}` : ''}
        </pre>
      )}
