...
function LegalTabs() {
  ...
  const initialTab = TABS.some((tb) => tb.id === searchParams.get('tab')) ? (searchParams.get('tab') as TabId) : 'privacy';
  const [tab, setTab] = useState<TabId>(initialTab);
  const { t } = useI18n();

  return (
    <div className="legal-wrap">
      <div className="legal-label">{t('legalPage.label', 'Legal')}</div>
      <h1 className="legal-h1">{t('legalPage.heading', 'Policies & terms.')}</h1>
