        ...
        <ul className="booking-list">
          {feed.candidates.map((c) => (
            <li key={c.slug} className={`booking-card${c.local ? ' booking-card-local' : ''}`}>
              ...
              <Link href={`/app/artists/${c.slug}`} className="booking-card-cta">{t('meBookingPage.viewCta', 'View')}</Link>
            </li>
          ))}
        </ul>
      )}
...
.booking-title { font-family: 'Bricolage Grotesque', sans-serif; font-weight: 800; font-size: 34px; line-height: 1.0; letter-spacing: -0.03em; color: var(--ink); margin: 10px 0 10px; }
.booking-sub { font-family: 'Work Sans', sans-serif; font-size: 15px; line-height: 1.6; color: var(--ink-a60); max-width: 56ch; margin: 0 0 24px; }
...
.booking-card-name { font-family: 'Work Sans', sans-serif; font-weight: 600; font-size: 16px; color: var(--ink); text-decoration: none; display: block; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.booking-card-name:hover { color: var(--role-venue); }
.booking-card-meta { font-family: 'Work Sans', sans-serif; font-size: 13px; color: var(--ink-a50); margin-top: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
.booking-card-cta { flex-shrink: 0; font-family: 'Work Sans', sans-serif; font-weight: 600; font-size: 13px; color: var(--ink-on-accent); background: var(--accent-grad-warm); border-radius: 9999px; padding: 9px 16px; text-decoration: none; }
.booking-empty { text-align: center; padding: 32px; background: var(--bg-2); border: 1px solid var(--line); border-radius: 16px; }
.booking-empty p { font-family: 'Work Sans', sans-serif; color: var(--ink-a60); margin: 0 0 14px; }
.booking-cta { display: inline-block; font-family: 'Work Sans', sans-serif; font-weight: 600; font-size: 15px; padding: 12px 22px; border-radius: 9999px; background: var(--accent-grad-warm); color: var(--ink-on-accent); text-decoration: none; }
`;
