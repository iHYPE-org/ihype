...
export default async function VenueBookingInboxPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!session?.user?.id) {
    redirect(`/login?callbackUrl=/app/me/venues/${slug}/booking-inbox`);
  }
  ...
  return (
    <div className="vbip-page">
      <div className="vbip-header">
        ...
        <Link className="vbip-back" href={`/app/me/venues/${profile.slug}/dashboard`}>{t('venuesSlugBookingInboxPage.backToDashboard', '← Dashboard')}</Link>
      </div>
      ...
      <style>{`
        ...
        .vbip-back { font-family: var(--font-mono); font-size: 0.7813rem; text-transform: uppercase; letter-spacing: .06em; color: var(--ink-a65); text-decoration: none; }
