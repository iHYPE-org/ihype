...
async function main() {
  ...
  const venue = await prisma.profile.upsert({
    ...
    update: {
      ...
      previousShowHighlights: 'Past runs here have sold through late, stretched into sunrise, and turned smaller showcases into repeat in-room bookings.',
  ...
  const chicagoVenue = await prisma.profile.upsert({
    ...
    update: {
      ...
      headline: 'Chicago open-air energy with enough room for a headline event and a neighborhood crowd.',
  ...
  const liveShow = await prisma.show.upsert({
    ...
    update: {
      ...
      description: 'A live back-to-back set with immersive lighting and a full warehouse crowd.',
  ...
  const rooftopShow = await prisma.show.upsert({
    ...
    update: {
      ...
      description: 'Sunset session on a downtown rooftop with guest lighting.',
  ...
  const chicagoLiveShow = await prisma.show.upsert({
    ...
    update: {
      ...
      description: 'Chicago riverfront show with a hometown crowd and skyline lighting.',
      status: ShowStatus.LIVE,
