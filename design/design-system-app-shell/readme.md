...
- **Advertiser** — third-party music-related organizations; manage ad campaigns only (no page creation, no posts)

**Radio has no host role.** Stations are generated: by genre, new, local, recommended by others, and from a listener's own history — a personal algorithm that stays on device and is never shared or sold.

**Promoters** are not a role — any account can share a HYPE Link — when a ticket buyer uses that link, the promoter earns a proportional share of the 10% promoter pool based on how much of the total gate their promotion drove.

### Products / Surfaces
- **iHYPE App (simplified)** — `templates/simple-app/` — the current frontend: three surfaces (MAP · MUSIC · ME) reached from the logo fan-out nav in the lower left. Fans have no public page; they have a dashboard (stats, tickets, history, recommendations, HYPE Link). Settings · Info · Legal · Accessibility live under ME.
- **Fan App (4-platform, legacy)** — primary consumer surface; one codebase, four frames: Desktop (browser), Mobile (generic), iOS (Dynamic Island, SF Pro), Android (gesture bar, Material density). 3-tab arch: Listen · Events · Pages
...
- **Role-flavored copy.** Each role (Fan, Artist, Venue, Advertiser) sees copy phrased for their context.
...
### Color
Deep, warm-black backgrounds. Almost no pure black — the base is `#0a0805`, a charred-wood brown-black. Four background tiers (bg → bg2 → bg3 → bg4) create subtle depth without gradients. The single accent is **iHYPE orange-red** (`#ff5029`) — vivid, high-energy, used for CTAs, the HYPE fire mechanic, and the Artist role. Role colors are distinct and saturated: Fan purple (`#b983ff`), Venue teal (`#22e5d4`), Advertiser amber (`#ffb84a`).
...
- **Verification copy pattern**: "Fan accounts are instant. Artist, Venue, and Advertiser accounts require verification — it protects everyone in the 70/20/10 ecosystem." (~48h review; see `guidelines/verification.card.html`)
...
### v8 — August 3, 2026 (simplification)
- **New primary frontend**: `templates/simple-app/` — three surfaces only (MAP · MUSIC · ME), navigated by tapping the iHYPE logo in the lower left; MAP/MUSIC/ME fan out from it as animated text (up-and-sideways, 45°, sideways) over a dimmed page. Tapping the logo, the scrim, or a destination collapses it; Escape closes.
- **Two-level fan-out nav**: the logo fans out MAP · MUSIC · ME. Only MUSIC has a second ring (Discover · Radio · Charts · Recommended · Playlists). MAP is driven by pinch/scroll zoom (the room list follows the viewport) with Search on the page; ME keeps its sections as in-dashboard accordion submenus.
- **Visual system (v8 polish)**: per-item generated cover art seeded off each name (8 curated 2-stop palettes × 4 motifs × varied angle/origin) so no two covers repeat; accent reserved for actions, hype values and live state (eyebrows/meta now neutral ink); chart breakdown bars use three tints of the accent (`--ih-t1/2/3`) instead of unrelated hues; a three-step elevation ladder (`--ih-e1` cards, `--ih-e2` player, `--ih-e3` overlays); inset player bar; and a fine grain + vignette layer over the whole app.
- **Artist & venue pages**: any artist or venue name is clickable anywhere in the app (Discover card, Charts, Recommended, Radio, MAP room list + day calendar, profile show rows, Me tickets/recs/history) and opens a profile sheet — bio, hypes/following, track list, upcoming dates with prices, iHYPE page URL, Hype + Follow. Hovering a name for ~340ms opens a compact popover (name, genre/city, bio, stats, "Open page ›"); Escape closes both.
- **No sales-pitch copy on signed-in screens** — the 70/20/10 + Stripe fee line appears only in a venue's ticket panel, after a show is tapped.
- **Now-playing player** is a fixed bottom bar starting to the right of the logo: prev / play / next, track, scrubber, elapsed, a draggable volume slider with mute, and a Queue button opening an "Up next" list (tap any row to jump; artist names link to their page). No cover thumbnail.
- **No page headers** — the modules are self-evident from the nav.
- **MAP is full-bleed**: the map fills the viewport above the footer, with Search rooms floating top-left and the selected-venue / rooms-in-view sheet floating right (drops to a bottom sheet under 820px).
- **Discover is full-screen** — the seed deck owns the viewport.
- **Light / dark mode** lives in Me → Settings → Appearance (theme tokens `--ih-bg`/`--ih-surface`/`--ih-ink-rgb`…).
- **MUSIC panes:**
  - **Discover** — the Seeds deck: 15–30s artist-uploaded previews on a drag/swipe card (pointer-tracked transform, SKIP/SAVE stamps, ←/→ keys); left skips, right saves to the Discover saves playlist.
  - **Radio** — generated stations (Genre · New · Local · From others · Your history).
  - **Charts** — Local / Regional / National / Global, genre chips, and a per-track hype breakdown bar (seeds · shows · radio + top city).
  - **Recommended** and **Playlists** — playlists support rename/track-removal, delete, and share to established HYPE-link connections.
- **DJ role removed.** Radio is now generated — Genre · New · Local · From others · Your history. The history station is a personal algorithm that stays on device and is never shared or sold.
- **Advertiser role added** (4th role, amber `#ffb84a`): campaign management only — no page creation.
- **Fans no longer have pages.** Everything a fan had on a page is now the ME dashboard: stats, tickets, purchase history, recommendations, and the HYPE Link they share to make connections.
- Settings · Info · Legal · Accessibility moved under ME as an accordion — opening one collapses the others to save screen space.
- **MAP is a real map**: Leaflet + OpenStreetMap (CARTO dark tiles), pinch/scroll zoom, venue pins scaled from local → city → regional → West Coast via a scope dropdown, plus a "jump to venue" dropdown. Tapping a pin opens that venue's day calendar (set times, prices, status) with per-set ticket buttons and a link to the venue page.
- **Search**: double-tap the logo anywhere (or the Search button on MAP) to open search over venues, tonight's sets, and radio stations.
- Wired to `lib/api.js` (mock by default): tickets, hype budget, hype send, ticket purchase. `templates/dj-kit/` retired.
...
### v4 — June 21, 2026
- **iHYPE App (simplified)** — `templates/simple-app/` — the current frontend: three surfaces (MAP · MUSIC · ME) reached from the logo fan-out nav in the lower left. Fans have no public page; they have a dashboard (stats, tickets, history, recommendations, HYPE Link). Settings · Info · Legal · Accessibility live under ME.
- **Fan App (4-platform, legacy)** — full rebuild of ui_kits/fan-app/ as a 7-file modular React app
