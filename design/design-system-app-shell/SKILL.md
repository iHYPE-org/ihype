...
**Promoter is not a role.** Any Fan, Artist, or Venue account can promote by sharing a HYPE Link/referral code; there's no Promoter signup or account type.
...
Core tokens to use: `--bg-base` (#0a0805), `--accent` (#ff5029), role colors Fan `#b983ff` / Artist `#ff5029` / Venue `#22e5d4` / Advertiser `#ffb84a` (promoting is built into every account, not a separate role or color). Type: Syne 800 for display, DM Sans for body, JetBrains Mono for eyebrows/metadata, Instrument Serif for editorial pull-quotes (Journal, About).
