...
function ArtistPage({ onToast }) {
  ...
  return (
    <div>
      <TimePeriodSelect value={period} onChange={setPeriod} />
      <StatGrid stats={[...getFanStats(period), { label: 'Monthly listeners', v: '12.4K', color: '#22e5d4' }, { label: 'Tickets sold', v: '218', color: '#ff5029' }, { label: 'Artist split', v: '$2,747', color: '#ff5029' }]} />
      <div style={{ marginBottom: 12, padding: '1rem', borderRadius: 16, border: '1px solid rgba(255,80,41,.2)', background: 'rgba(255,80,41,.06)' }}>
        <div style={{ fontFamily: 'var(--f-d)', fontWeight: 800, fontSize: '3rem', letterSpacing: '-.04em', color: 'var(--accent)', lineHeight: 1 }}>70%</div>
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 12, padding: '1rem', borderRadius: 16, border: '1px solid rgba(34,229,212,.2)', background: 'rgba(34,229,212,.06)' }}>
        <div style={{ fontFamily: 'var(--f-d)', fontWeight: 800, fontSize: '3rem', letterSpacing: '-.04em', color: '#22e5d4', lineHeight: 1 }}>20%</div>
...
function CreatePageFlow({ onToast, onDone }) {
  const ROLES = [
    ...
    { id:'Artist', icon:'🎤', tint:'#ff5029', desc:'70% of every ticket, Tour Creator, media.' },
        ...
        <div>
          ...
          {(role==='Artist'||role==='Venue') && (
            <div style={{ padding:'13px 16px', borderRadius:14, border:`1px solid ${rf.tint}33`, background:`${rf.tint}06`, marginBottom:14 }}>
              <div style={{ fontFamily:'var(--f-d)', fontWeight:800, fontSize:'2rem', color:rf.tint, lineHeight:1 }}>{role==='Artist'?'70%':'20%'}</div>
...
function DJPublicProfile({ dj, onBack }) {
  const shows = [
...
function RoleSwitcherSheet({ open, currentRole, onClose, onSwitch }) {
  ...
  const roles = [
    ...
    { id: 'Artist', icon: '🎤', color: '#ff5029', desc: '70% split + tour creator' },
