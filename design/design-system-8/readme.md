...
**Promoters** are Fans, Artists, or Venues who share a referral link — when a ticket buyer uses that link, the promoter earns a proportional share of the 10% promoter pool based on how much of the total gate their promotion drove.
...
### Color
Ink-navy backgrounds. Almost no pure black — the base is `#0b1220`, a deep blue-black. Four background tiers (`--bg-base` → `--bg-surface` → `--bg-raised` → `--bg-overlay`) create depth without gradients. The single accent is **iHYPE orange-red** (`#ff5029`) — vivid, high-energy, used for CTAs, the HYPE fire mechanic, and the Artist role. Role colors are distinct and saturated: Fan violet (`#b983ff`), Venue teal (`#22e5d4`), Advertiser amber (`#ffb84a`). Promoter pink (`#ff3e9a`) colors the 10% pool slice only — it is not an account type.

The accent and the four role hues are the only things that survived the 2026-08-07 overhaul unchanged; the ground and the ink both moved from warm to cool.

### Typography
- **Bricolage Grotesque** — display face, replacing Syne. Variable, with an optical-size axis: feed `opsz` the pixel size and set `--tracking-display` (−.035em) at headline sizes, or large text looks loose and small text cramped. Weight 800 for headlines and the wordmark, 600 for nav pills and card titles.
- **Work Sans** — body and UI text, replacing DM Sans. Weight 400/500/600. Workhorse for content, buttons, navigation labels.
- **JetBrains Mono** — eyebrows, timestamps, metadata chips, tabular figures. All-caps, 9–12px, tracked out to 0.14–0.22em. Creates a "technical readout" aesthetic.
- **Instrument Serif** — editorial / pull-quote use only. Italic form. Rarely used.

### Backgrounds
Dark is canonical; a light theme ships as opt-in `[data-theme="light"]` on cool paper (`#f4f6fa`) and is axe-verified in the product. Background layers provide depth. No full-bleed photography. Album art uses square aspect-ratio panels with large border-radius and vivid gradient overlays sourced from the accent/role color.
...
components/shell/   ← Music · Map · Me app-shell chrome (added v8)
  LogoTrigger ← the 76px accent squircle; the only persistent nav affordance
  ArcNav      ← the radial fan; hand-placed ARC table, two breakpoints
  PlayerPill  ← persistent mini player, artwork shrinks it, names link out
  FullPlayer  ← phone only: full-screen player behind the bar's artwork
  ...
  MapSheet    ← pin detail sheet; belongs to the map, closes when you leave it
