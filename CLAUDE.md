...
- **Never call the Prisma MCP server's `migrate-dev` tool.** `.mcp.json` registers `prisma` (stdio, `npx prisma mcp`), which exposes three tools: `migrate-status` (read-only, allowlisted in `.claude/settings.json`), `Prisma-Studio` (local UI, useless in a remote session), and `migrate-dev`. `migrate-dev` writes a new migration straight into `prisma/migrations/` — the directory that auto-deploys on every push to `main` — and can reset/drop the database it points at. It bypasses the gating workflow above entirely. It is deliberately **not** allowlisted so it prompts every time; write migrations by hand into `prisma/migrations-pending/` instead. **For reading data, use the Supabase MCP server, not Prisma's** — the Prisma one has no query tool at all.
- **Do not add Prisma's *remote* MCP server (`https://mcp.prisma.io/mcp`) — it is a different product.** Prisma ships two MCP servers, and only the local one (`npx prisma mcp`, registered above) is relevant here. The remote one is the management API for **Prisma Postgres**, Prisma's own hosted database product, and authenticates via OAuth to a Prisma Console workspace on first use. Its tools (`CreateDatabase`/`DeleteDatabase`/`ListDatabases`, `CreateBackup`/`ListBackups`/`CreateRecovery`, `Create`/`Delete`/`ListConnectionStrings`, `ExecuteSqlQuery`, `IntrospectSchema`, `search_prisma_documentation`) act on databases inside that workspace. iHYPE's database is Supabase Postgres behind Cloudflare Hyperdrive — there is no Prisma Postgres instance for any of those tools to reach, so the only thing connecting it would add is an OAuth-gated, write-capable database surface (`ExecuteSqlQuery`, `DeleteDatabase`) sitting outside both the `read_only=true` pin below and the `migrations-pending/` gating workflow above. Note also that adding it the documented way — claude.ai → Connect apps → **+ Add integration** — installs it as a per-account connector, so it would **not** appear in `.mcp.json` and this repo would have no record of it; that is why the decision is written down here. If iHYPE ever actually moves onto Prisma Postgres, revisit this bullet.
