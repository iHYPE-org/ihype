...
**`npm run check:backup-cadence` is what keeps this number honest**, and it runs nightly. It measures the delivered gaps rather than reading the cron, warns past the 8 h target (GitHub's queue is nobody here's to clear) and fails past 12 h or if the workflow stops firing at all. The schedule moved off `:00` to `:37` on 2026-09-07 because the top of the hour is GitHub's most congested minute — **that is a hypothesis about the queue, not a measurement.** Re-run the probe after a week and lower this number only if it earns it. **RE-RUN 2026-09-13, six days after the move, and it does NOT earn a lower number — but the move helped, and the worst case halved.** Over the 24 gaps since 2026-09-07 the delivered spacing is **min 4.27 h · median 6.56 h · max 8.77 h**, with **3 of 24 past the 8 h target and none past the 12 h ceiling**; the delay against the slot is essentially unchanged (median 4.25 h against 4.14 h before), so `:37` did not shorten the queue — it removed the long tail. Across the whole history the probe still reports a 16.70 h worst case, and **that gap is 2026-09-06 → 09-07, before the move**: quoting it as current overstates the risk by a factor of two. **Eight hours stays the promise and is now missed about one time in eight rather than one in two.** Lowering it needs a week where nothing exceeds it; raising it to nine would be defensible today and is not worth the churn. PITR would make that seconds. This is a recorded, accepted risk of running without capital — it is a worse RPO, not an absent recovery capability, and it is reviewed when the platform starts holding real money.
