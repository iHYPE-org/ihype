...
| 101 | Discover / FanHome (Listen) / Events list views | **SoundCloud-inspired mobile UX review (2026-07-03) — still needs `.dc.html` design work.** Audit list/card rows for one consistent row height and layout (avatar, title, one line of metadata, right-aligned action) — SoundCloud keeps every feed row the same shape for fast thumb-scanning. Current card density varies across `/discover`, `/listen`, `/shows` in the live app. Left as design work since it spans many pages' visual layout, unlike rows 100/102 which were wiring/component-reuse fixes. | — | ⏳ Pending |
