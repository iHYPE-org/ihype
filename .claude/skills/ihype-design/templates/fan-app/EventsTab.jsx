  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 80, background: 'rgba(0,0,0,.6)', backdropFilter: 'blur(6px)' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'var(--bg-2)', borderRadius: '22px 22px 0 0', padding: '1.25rem 1.25rem 2.5rem' }}>
        ...
        <p style={{ fontFamily: 'var(--f-m)', fontSize: '.62rem', color: 'var(--ink-3)', textAlign: 'center', marginTop: 6, opacity:.8 }}>{window.ihypeT ? window.ihypeT('ckAgreeTerms') : 'By buying you agree to the'} <a href="https://ihype.org/terms" target="_blank" style={{color:'var(--ink-3)'}}>{window.ihypeT ? window.ihypeT('ckTerms') : 'Terms'}</a>.</p>
      </div>
    </div>
  );
}
