...
function CheckoutSheet({ show, onClose, onDone }) {
  ...
  const [status, setStatus] = React.useState('idle'); // idle | paying | error
  React.useEffect(() => { if (show) setStatus('idle'); }, [show]);
  if (!show) return null;
  const gross = show.price * qty;
  const fee = gross * 0.029 + 0.3;
  const total = (gross + fee).toFixed(2);
  const tm = (show.price * 1.27 * qty).toFixed(2);
  const pay = async () => {
    setStatus('paying');
    try {
      if (window.IHYPE_PURCHASE) await window.IHYPE_PURCHASE(show.id, null);
      onDone();
    } catch (e) {
      setStatus('error');
    }
    ...
    <div style={{ position: 'absolute', inset: 0, zIndex: 80, background: 'rgba(0,0,0,.6)', backdropFilter: 'blur(6px)' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'var(--bg-2)', borderRadius: '22px 22px 0 0', padding: '1.25rem 1.25rem 2.5rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 12px', borderRadius: 11, background: 'var(--bg-3)', marginBottom: 14 }}>
          <span style={{ fontFamily: 'var(--f-m)', fontSize: '.78rem', color: 'var(--ink-3)' }}>{window.ihypeT ? window.ihypeT('ckTicketmaster') : 'Ticketmaster'}: ${tm}</span>
          <span style={{ fontFamily: 'var(--f-d)', fontWeight: 800, color: '#22e5d4' }}>{window.ihypeT ? window.ihypeT('ckYouPay') : 'You pay'} ${total}</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '.72rem', color: 'var(--ink-3)', padding: '0 2px', marginBottom: 16 }}>
          <span>{window.ihypeT ? window.ihypeT('faceValue') : 'Face value'} (qty {qty})</span><span>${gross.toFixed(2)}</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '.72rem', color: 'var(--ink-3)', padding: '0 2px', marginTop: -12, marginBottom: 16 }}>
          <span>{window.ihypeT ? window.ihypeT('cardProcessing') : 'Card processing (Stripe, at cost)'}</span><span>+${fee.toFixed(2)}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
          <span style={{ fontFamily: 'var(--f-m)', fontSize: '.78rem', color: 'var(--ink-3)' }}>{window.ihypeT ? window.ihypeT('qty') : 'Qty'}</span>
        ...
        {status === 'error' && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', borderRadius: 10, background: 'rgba(255,69,69,.1)', border: '1px solid rgba(255,69,69,.3)', marginBottom: 12 }}>
            <span style={{ fontSize: '.9rem' }}>⚠</span>
            <span style={{ fontFamily: 'var(--f-b)', fontSize: '.78rem', color: '#ff8080' }}>{window.ihypeT ? window.ihypeT('ckPaymentFailed') : 'Payment failed — check your card and try again.'}</span>
          </div>
        )}
        <button onClick={pay} disabled={status === 'paying'} style={{ width: '100%', padding: '13px', borderRadius: 999, background: 'var(--accent)', color: '#fff', border: 'none', fontFamily: 'var(--f-d)', fontWeight: 800, fontSize: '.95rem', cursor: status === 'paying' ? 'default' : 'pointer', boxShadow: '0 4px 20px rgba(255,80,41,.3)', opacity: status === 'paying' ? .7 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
          {status === 'paying' && <span style={{ width: 14, height: 14, borderRadius: '50%', border: '2px solid rgba(255,255,255,.35)', borderTopColor: '#fff', animation: 'ihype-spin .7s linear infinite', display: 'inline-block' }} />}
          {status === 'paying' ? (window.ihypeT ? window.ihypeT('ckProcessing') : 'Processing…') : (window.ihypeT ? window.ihypeT('ckPayApplePay').replace('{amount}', '$'+total) : ('Pay $'+total+' · Apple Pay'))}
        </button>
        <style>{'@keyframes ihype-spin{to{transform:rotate(360deg)}}'}</style>
        <p style={{ fontFamily: 'var(--f-m)', fontSize: '.68rem', color: 'var(--ink-3)', textAlign: 'center', marginTop: 8 }}>{window.ihypeT ? window.ihypeT('ckFeeCharter') : 'iHYPE fee $0 · 70% to artist · locked in charter'}</p>
        <p style={{ fontFamily: 'var(--f-m)', fontSize: '.62rem', color: 'var(--ink-3)', textAlign: 'center', marginTop: 6, opacity:.8 }}>{window.ihypeT ? window.ihypeT('ckAgreeTerms') : 'By buying you agree to the'} <a href="https://ihype.app/terms" target="_blank" style={{color:'var(--ink-3)'}}>{window.ihypeT ? window.ihypeT('ckTerms') : 'Terms'}</a>.</p>
      </div>
    </div>
  );
}
...
function FilterSheet({ open, filter, onFilter, onClose }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 70, background: 'rgba(0,0,0,.5)', backdropFilter: 'blur(6px)' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'var(--bg-2)', borderRadius: '22px 22px 0 0', padding: '1.25rem 1.25rem 2.5rem' }}>
        <div style={{ width: 36, height: 4, borderRadius: 999, background: 'var(--line)', margin: '0 auto 18px' }} />
        <div style={{ fontFamily: 'var(--f-d)', fontWeight: 800, fontSize: '1.1rem', marginBottom: 16 }}>{window.ihypeT ? window.ihypeT('filterEvents') : 'Filter events'}</div>
        <div style={{ fontFamily: 'var(--f-m)', fontSize: '.7rem', letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--ink-3)', marginBottom: 8 }}>{window.ihypeT ? window.ihypeT('genre') : 'Genre'}</div>
        ...
        <div style={{ fontFamily: 'var(--f-m)', fontSize: '.7rem', letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--ink-3)', marginBottom: 8 }}>{(window.ihypeT ? window.ihypeT('maxPrice') : 'Max price')}: ${maxPrice}</div>
        ...
        <button onClick={onClose} style={{ width: '100%', padding: '11px', borderRadius: 999, background: 'var(--accent)', color: '#fff', border: 'none', fontFamily: 'var(--f-d)', fontWeight: 800, fontSize: '.88rem', cursor: 'pointer' }}>{window.ihypeT ? window.ihypeT('ckApplyFilters') : 'Apply filters'}</button>
      </div>
    </div>
  );
}
      ...
      <div style={{ padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 }}>
            ...
            <span style={{ fontFamily: 'var(--f-m)', fontSize: '.7rem', color: '#22e5d4' }}>+ $0 iHYPE fees</span>
    ...
    <div>
      ...
      {D.fanReceipts.length === 0 && tab2 === 'upcoming' && (
        <div style={{ textAlign:'center', padding:'3rem 1rem' }}>
          ...
          <div style={{ fontFamily:'var(--f-d)', fontWeight:800, fontSize:'1rem', marginBottom:8 }}>{window.ihypeT ? window.ihypeT('noUpcomingShows') : 'No upcoming shows'}</div>
          <div style={{ fontFamily:'var(--f-m)', fontSize:'.78rem', color:'var(--ink-3)', marginBottom:16 }}>{window.ihypeT ? window.ihypeT('hypeToUnlock') : 'Hype artists in Seeds to unlock early ticket access.'}</div>
          <button onClick={onBrowse} style={{ padding:'10px 22px', borderRadius:999, background:'var(--accent)', color:'#fff', border:'none', fontFamily:'var(--f-d)', fontWeight:800, fontSize:'.85rem', cursor:'pointer' }}>{window.ihypeT ? window.ihypeT('browseLocalEvents') : 'Browse local events →'}</button>
        </div>
      )}
      {D.fanReceipts.length === 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '3rem 1rem', gap: 14, textAlign: 'center' }}>
          ...
          <div style={{ fontFamily: 'var(--f-d)', fontWeight: 800, fontSize: '1rem' }}>{window.ihypeT ? window.ihypeT('noTicketsYet') : 'No tickets yet.'}</div>
          <div style={{ fontFamily: 'var(--f-b)', fontSize: '.82rem', color: 'var(--ink-3)', lineHeight: 1.6, maxWidth: '28ch' }}>{window.ihypeT ? window.ihypeT('findShowShort') : 'Find a show you love, hype the artist, and buy direct — iHYPE takes $0.'}</div>
          <button style={{ padding: '9px 22px', borderRadius: 999, background: 'var(--accent)', color: '#fff', border: 'none', fontFamily: 'var(--f-d)', fontWeight: 800, fontSize: '.85rem', cursor: 'pointer' }}>{window.ihypeT ? window.ihypeT('browseEvents') : 'Browse events →'}</button>
        </div>
      )}
      {D.fanReceipts.map(r => {
        ...
        return (
          <div key={r.id} onClick={() => window.openIHYPETicketQR && window.openIHYPETicketQR(r)} style={{ padding: '1rem', borderRadius: 16, border: `1px solid ${isTonight ? r.tint + '55' : 'var(--line)'}`, background: `linear-gradient(135deg,${r.tint}0d,transparent)`, marginBottom: 10, cursor: 'pointer' }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
              <button onClick={() => window.openIHYPETransfer && window.openIHYPETransfer()} style={{ flex: 1, padding: '8px', borderRadius: 10, border: '1px solid var(--line)', background: 'transparent', color: 'var(--ink-2)', fontFamily: 'var(--f-m)', fontSize: '.75rem', cursor: 'pointer' }}>{window.ihypeT ? window.ihypeT('transfer') : 'Transfer →'}</button>
              <button onClick={() => window.openIHYPEPostShow && window.openIHYPEPostShow({ artist: r.artist, venue: r.event })} style={{ flex: 1, padding: '8px', borderRadius: 10, border: '1px solid var(--line)', background: 'transparent', color: 'var(--ink-2)', fontFamily: 'var(--f-m)', fontSize: '.75rem', cursor: 'pointer' }}>{window.ihypeT ? window.ihypeT('rateShow') : 'Rate show ★'}</button>
            </div>
          </div>
        );
      })}
  ...
  return (
    <div>
      <div style={{ position: 'relative', marginBottom: 14 }}>
        <input value={q} onChange={e => setQ(e.target.value)} placeholder={window.ihypeT ? window.ihypeT('searchEvents') : 'Search events, venues, artists…'} style={{ width: '100%', padding: '10px 14px 10px 38px', borderRadius: 12, border: '1px solid var(--line)', background: 'var(--bg-3)', color: 'var(--ink)', fontFamily: 'var(--f-b)', fontSize: '.88rem', outline: 'none', boxSizing: 'border-box' }} />
      ...
      {ageGate && (
        <div style={{ position:'absolute', inset:0, zIndex:80, background:'rgba(0,0,0,.7)', display:'flex', alignItems:'flex-end' }}>
          <div style={{ width:'100%', background:'var(--bg-2)', borderRadius:'22px 22px 0 0', padding:'1.5rem 1.25rem 2.5rem' }}>
            ...
            <div style={{ fontFamily:'var(--f-d)', fontWeight:800, fontSize:'1.1rem', textAlign:'center', marginBottom:8 }}>{window.ihypeT ? window.ihypeT('ageVerifyTitle') : 'Age Verification Required'}</div>
            <div style={{ fontFamily:'var(--f-b)', fontSize:'.82rem', color:'var(--ink-3)', textAlign:'center', lineHeight:1.6, marginBottom:20 }}>{window.ihypeT ? window.ihypeT('ageVerifyBody') : 'This event requires attendees to be 21+. Please confirm your age to continue.'}</div>
            <button onClick={()=>{ setCheckout(ageGate); setAgeGate(null); }} style={{ width:'100%', padding:'13px', borderRadius:999, background:'var(--accent)', color:'#fff', border:'none', fontFamily:'var(--f-d)', fontWeight:800, fontSize:'.95rem', cursor:'pointer', marginBottom:10 }}>{window.ihypeT ? window.ihypeT('confirm21') : 'I confirm I am 21+ →'}</button>
            <button onClick={()=>setAgeGate(null)} style={{ width:'100%', padding:'11px', borderRadius:999, border:'1px solid var(--line)', background:'transparent', color:'var(--ink-3)', fontFamily:'var(--f-m)', fontSize:'.88rem', cursor:'pointer' }}>{window.ihypeT ? window.ihypeT('cancel') : 'Cancel'}</button>
          </div>
        </div>
      )}
