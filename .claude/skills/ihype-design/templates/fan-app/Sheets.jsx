...
function PostPurchaseMoment({ show, onClose }) {
  ...
  if (!show) return null;
  const cells = [
    { amt: '$12.60', pct: '70%', who: 'Artist', color: '#ff5029', show: step >= 1 },
    { amt: '$3.60', pct: '20%', who: 'Venue', color: '#22e5d4', show: step >= 2 },
    { amt: '$1.80', pct: '10%', who: 'Promoters', color: '#b983ff', show: step >= 3 },
    { amt: '$0.00', pct: '0%', who: 'iHYPE', color: '#22e5d4', show: step >= 3 },
    { amt: '+$0.82', pct: 'card fee', who: 'Stripe (at cost)', color: '#5a5048', show: step >= 3 },
  ];
  return (
...
const FAQ = [
  ...
  { q: 'How does the 70/20/10 split work?', a: "Every ticket: 70% to the artist, 20% to the venue, 10% to the promoter pool — split among everyone whose referral link drove a purchase. iHYPE takes 0% — the only extra charge is the unavoidable card-processing fee (2.9% + $0.30; AMEX 3.5% + $0.30), passed through at cost." },
...
function ChangelogSheet({ open, onClose }) {
  ...
  const entries = [
    ...
    { v: '0.1.0-beta.4', d: 'Jun 20, 2026', items: ['3-tab architecture: Listen · Events · Pages', 'Seeds swipe deck with gesture hint', 'Checkout with TM price comparison', 'Post-purchase 70/20/10 payout reveal', 'Notification primer after first ticket purchase'] },
